...
(function (window, document, location, $, undefined) {
  "use strict";
var _twoj_slideshowClass = 'twoj_slideshow',
    _fullscreenClass = 'fullscreen',

    wrapClass = _twoj_slideshowClass + '__wrap',
    ...
    stageClass = _twoj_slideshowClass + '__stage',
    stageFrameClass = stageClass + '__frame',
    stageFrameVideoClass = stageFrameClass + '--video',
    stageShaftClass = stageClass + '__shaft',

    grabClass = _twoj_slideshowClass + '__grab',
    pointerClass = _twoj_slideshowClass + '__pointer',

    arrClass = _twoj_slideshowClass + '__arr',
    arrDisabledClass = arrClass + '--disabled',
    arrPrevClass = arrClass + '--prev',
    arrNextClass = arrClass + '--next',
    arrArrClass = arrClass + '__arr',

    navClass = _twoj_slideshowClass + '__nav',
    ...
    fadeClass = _twoj_slideshowClass + '__fade',
    fadeFrontClass = fadeClass + '-front',
    fadeRearClass = fadeClass + '-rear',

    shadowClass = _twoj_slideshowClass + '__shadow',
    shadowsClass = shadowClass + 's',
    shadowsLeftClass = shadowsClass + '--left',
    shadowsRightClass = shadowsClass + '--right',

    activeClass = _twoj_slideshowClass + '__active',
    selectClass = _twoj_slideshowClass + '__select',

    hiddenClass = _twoj_slideshowClass + '--hidden',

    fullscreenClass = _twoj_slideshowClass + '--fullscreen',
    fullscreenIconClass = _twoj_slideshowClass + '__fullscreen-icon',

    errorClass = _twoj_slideshowClass + '__error',
    loadingClass = _twoj_slideshowClass + '__loading',
    loadedClass = _twoj_slideshowClass + '__loaded',
    loadedFullClass = loadedClass + '--full',
    loadedImgClass = loadedClass + '--img',

    grabbingClass = _twoj_slideshowClass + '__grabbing',

    imgClass = _twoj_slideshowClass + '__img',
    imgFullClass = imgClass + '--full',

    dotClass = _twoj_slideshowClass + '__dot',
    thumbClass = _twoj_slideshowClass + '__thumb',
    thumbBorderClass = thumbClass + '-border',

    htmlClass = _twoj_slideshowClass + '__html',

    videoClass = _twoj_slideshowClass + '__video',
    videoPlayClass = videoClass + '-play',
    videoCloseClass = videoClass + '-close',

    captionClass = _twoj_slideshowClass + '__caption',
    captionWrapClass = _twoj_slideshowClass + '__caption__wrap',

    spinnerClass = _twoj_slideshowClass + '__spinner';
...
function getVideoThumbs (dataFrame, data, twoj_slideshow) {
  ...
  if (video.type === 'youtube') {
    ...
    $.ajax({
      ...
      success: function (json) {
        dataFrame.thumbsReady = true;
        updateData(data, {img: json[0].thumbnail_large, thumb: json[0].thumbnail_small}, dataFrame.i, twoj_slideshow);
      }
    });
  } else {
    dataFrame.thumbsReady = true;
...
function updateData (data, _dataFrame, i, twoj_slideshow) {
  for (var _i = 0, _l = data.length; _i < _l; _i++) {
    ...
    if (dataFrame.i === i && dataFrame.thumbsReady) {
      ...
      twoj_slideshow.splice(_i, 1, $.extend(
          {},
          dataFrame,
          clear,
          _dataFrame
...
function getDataFromHtml ($el) {
  ...
  function getDataFromImg ($img, imgData, checkVideo) {
	...
	//imgData._imgLink = $img.data('link');
	/*$child._imgLink = $img.data('link');*/
...
$.twoj_slideshow = function ($twoj_slideshow, opts) {
  $HTML = $HTML || $('html');
  $BODY = $BODY || $('body');

  var that = this,
      stamp = $.now(),
      stampClass = _twoj_slideshowClass + stamp,
      twoj_slideshow = $twoj_slideshow[0],
      data,
      dataFrameCount = 1,
      twoj_slideshowData = $twoj_slideshow.data(),
      size,
  ...
  twoj_slideshowData.twoj_slideshow = this;
  ...
  function bindGlobalEvents (FLAG) {
    var keydownCommon = 'keydown.' + _twoj_slideshowClass,
        keydownLocal = 'keydown.' + _twoj_slideshowClass + stamp,
        resizeLocal = 'resize.' + _twoj_slideshowClass + stamp;
  ...
  function appendElements (FLAG) {
    ...
    if (FLAG) {
      $twoj_slideshow
          .html('')
          .addClass(_twoj_slideshowClass + ' ' + stampClass)
          .append($wrap)
          .before($style)
          .before($anchor);
      ...
      $anchor.detach();
      $twoj_slideshow
          .html(twoj_slideshowData.urtext)
  ...
  function setData () {
    data = that.data = data || clone(opts.data) || getDataFromHtml($twoj_slideshow);
  ...
  function setOptions () {
    ...
    if (o_navThumbs) {
      ...
      setStyle($style, $.twoj_slideshow.jst.style({w: o_thumbSide, h: o_thumbSide2, b: opts.thumbborderwidth, m: opts.thumbmargin, s: stamp, q: !COMPAT}));
  ...
  function loadImg (indexes, type, specialMeasures, specialFit, again) {
    eachIndex(indexes, type, function (i, index, dataFrame, $frame, key, frameData) {
      ...
      function error () {
        ...
        $.twoj_slideshow.cache[src] = 'error';
      ...
      function loaded () {
        ...
        //console.log('$.twoj_slideshow.measures[src]', $.twoj_slideshow.measures[src]);

        $.twoj_slideshow.measures[src] = imgData.measures = $.twoj_slideshow.measures[src] || {
          width: img.width,
          height: img.height,
...
////console.log(imgData);
        ...
        $.twoj_slideshow.cache[src] = frameData.state = 'loaded';
      ...
      if (!$.twoj_slideshow.cache[src]) {
        $.twoj_slideshow.cache[src] = '*';
        ...
        (function justWait () {
          if ($.twoj_slideshow.cache[src] === 'error') {
            error();
          } else if ($.twoj_slideshow.cache[src] === 'loaded') {
  ...
  function updatetwoj_slideshowState () {
  ...
  function triggerEvent (event, extra) {
    $twoj_slideshow.trigger(_twoj_slideshowClass + ':' + event, [that, extra]);
  }
  ...
  that.requestFullScreen = function () {
    if (o_allowFullScreen && !that.fullScreen) {
      ...
      if (o_nativeFullScreen) {
        fullScreenApi.request(twoj_slideshow);
      }
  ...
  function cancelFullScreen () {
    if (that.fullScreen) {
      ...
      if (FULLSCREEN) {
        fullScreenApi.cancel(twoj_slideshow);
      }
    ...
    if (typeof video === 'object' && dataFrame.videoReady) {
      ...
      waitFor(function () {
        ...
        if (_activeIndex === activeIndex) {
          dataFrame.$video = dataFrame.$video || $($.twoj_slideshow.jst.video(video));
  ...
  stageShaftTouchTail = moveOnTouch($stageShaft, {
    ...
    onEnd: function (result) {
      ...
      if ( !result.moved && !result.control && result.pos==0 && result.newPos==0 ){
      	if(  	result.startEvent.target.className=='twoj_slideshow__html' || 
      			result.startEvent.target.className=='twoj_slideshow__img'
      	){
      		var link = twoj_slideshowData.twoj_slideshow.activeFrame.link;
      		var link_targer = twoj_slideshowData.twoj_slideshow.activeFrame.link_targer;
      		
      		if( typeof  link != 'undefined' && link != "" ){
      			if( link_targer=='javascript' ){ 
      				eval(link);
      			} else {
      				if( link_targer!="_blank" ) link_targer="_self";
      				window.open(link, link_targer);
      				return ;
      			}      			
      		}
      	}
...
$.fn.twoj_slideshow = function (opts) {
  return this.each(function () {
    var that = this,
        $twoj_slideshow = $(this),
        twoj_slideshowData = $twoj_slideshow.data(),
        twoj_slideshow = twoj_slideshowData.twoj_slideshow;

    if (!twoj_slideshow) {
      waitFor(function () {
        return !isHidden(that);
      }, function () {
        twoj_slideshowData.urtext = $twoj_slideshow.html();
        new $.twoj_slideshow($twoj_slideshow,
            /* Priority for options:
             * 1. <div data-loop="true"></div>
             * 2. $('div').twoj_slideshow({loop: false})
             * 3. Defaults */
            $.extend(
                {},
                OPTIONS,
                window.twoj_slideshowDefaults,
                opts,
                twoj_slideshowData
            )
        );
      ...
      twoj_slideshow.setOptions(opts);
    }
  });
};
$.twoj_slideshow.instances = [];

function calculateIndexes () {
  $.each($.twoj_slideshow.instances, function (index, instance) {
    instance.index = index;
  });
}

function addInstance (instance) {
  $.twoj_slideshow.instances.push(instance);
  calculateIndexes();
}

function hideInstance (instance) {
  $.twoj_slideshow.instances.splice(instance.index, 1);
  calculateIndexes();
}
$.twoj_slideshow.cache = {};
$.twoj_slideshow.measures = {};
$ = $ || {};
$.twoj_slideshow = $.twoj_slideshow || {};
$.twoj_slideshow.jst = $.twoj_slideshow.jst || {};

$.twoj_slideshow.jst.style = function(v) {
...
$.twoj_slideshow.jst.video = function(v) {
var __t, __p = '', __e = _.escape, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, ''); }
__p += '<div class="twoj_slideshow__video"><iframe src="';
 print((v.type == 'youtube' ? 'https://youtube.com/embed/' + v.id +'?autoplay=1' : v.type == 'vimeo' ? 'https://player.vimeo.com/video/' + v.id + '?autoplay=1&badge=0' : v.id)  + (v.s && v.type != 'custom' ? '&' + v.s : '')) ;
__p += '" frameborder="0" allowfullscreen></iframe></div>';
return __p;
};
$(function () {
  $('.' + _twoj_slideshowClass + ':not([data-auto="false"])').twoj_slideshow();
});
})(window, document, location, typeof rbjQuer !== 'undefined' && rbjQuer ? rbjQuer : jQuery );

(function($) {
	$('.twoj_slideshow').each( function(){
		var objectOptions = window[ $(this).data('options') ];
		console.log(objectOptions);
		console.log(this);
		var realOptions = $.extend({},objectOptions);
		$(this).twoj_slideshow( realOptions ); 
	});
})( typeof rbjQuer !== 'undefined' && rbjQuer ? rbjQuer : jQuery  );
